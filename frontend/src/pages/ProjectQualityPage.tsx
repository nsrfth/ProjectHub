...
import { NcrRegister } from '@/features/quality/NcrRegister';
...
// v2.5 (PMIS R9): per-project quality / NCR register.
  ...
  if (!teamId || !project) {
    return (
      <div className="p-8">
        ...
        <p className="mt-4 text-sm text-slate-500">{t('ncr.notFound')}</p>
      </div>
    );
  }
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6 gap-4">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold truncate">{t('ncr.title')}</h1>
      ...
      <section className="bg-surface rounded shadow p-6">
        <NcrRegister teamId={teamId} projectId={projectId!} canManage={canManage} />
      </section>
    </div>
  );
}
