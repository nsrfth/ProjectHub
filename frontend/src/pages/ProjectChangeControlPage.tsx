...
import { ChangeRequestRegister } from '@/features/changeControl/ChangeRequestRegister';
...
// v2.5 (PMIS R9): per-project change request register.
// Reads need project READ; writes need change.manage; decide/apply need change.approve.
export default function ProjectChangeControlPage(): JSX.Element {
  ...
  if (!teamId || !project) {
    return (
      <div className="p-8">
        ...
        <p className="mt-4 text-sm text-slate-500">{t('changeControl.notFound')}</p>
      </div>
    );
  }
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6 gap-4">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold truncate">{t('changeControl.title')}</h1>
      ...
      <section className="bg-surface rounded shadow p-6">
        <ChangeRequestRegister teamId={teamId} projectId={projectId!} canManage={canManage} />
      </section>
    </div>
  );
}
