...
import { RecordsRegister } from '@/features/records/RecordsRegister';
...
// v2.4 (PMIS R8): per-project record register (Issues, RFIs, Documents,
// Stakeholders, MoM + custom types). Reads need project READ; writes need
// project WRITE (enforced by the backend).
export default function ProjectRecordsPage(): JSX.Element {
  ...
  if (!teamId || !project) {
    return (
      <div className="p-8">
        ...
        <p className="mt-4 text-sm text-slate-500">{t('records.notFound')}</p>
      </div>
    );
  }
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6 gap-4">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold truncate">{t('records.title')}</h1>
      ...
      <section className="bg-surface rounded shadow p-6">
        <RecordsRegister teamId={teamId} projectId={projectId!} canManage={canManage} />
      </section>
    </div>
  );
}
