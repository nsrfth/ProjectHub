...
// v2.5 (PMIS R9): per-project risk register. Routes through project access
// (any member with READ may view; writes need the risk.manage permission, which
// the backend enforces — managers get it by default).
export default function ProjectRiskPage(): JSX.Element {
  ...
  if (!teamId || !project) {
    return (
      <div className="p-8">
        ...
        <p className="mt-4 text-sm text-slate-500">{t('risk.notFound')}</p>
      </div>
    );
  }
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6 gap-4">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold truncate">{t('risk.title')}</h1>
      ...
      <section className="bg-surface rounded shadow p-6">
        <RiskRegister teamId={teamId} projectId={projectId!} canManage={canManage} />
      </section>
    </div>
  );
}
