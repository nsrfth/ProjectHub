...
| Budget / cost report (v1.71) | **`GET /reports/budget`** + **`/budget.csv`**. `ReportsService.budgetReport(teamId)` → `{ projects, rollupByCurrency }`. Per-project rows from `Project.plannedBudget` / `actualSpent` / `budgetCurrency` (no task rollup in v1). Decimal math in `lib/budgetReportMath.ts` — variance = planned − actual, utilization null when planned is 0/null. Rollup groups by currency; never sums across IRR/EUR/USD. Projects with neither budget field show `hasBudget: false`. Read-only. |
